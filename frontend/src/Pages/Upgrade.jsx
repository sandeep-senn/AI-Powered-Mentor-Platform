...
import { Sparkles, Check, Rocket, Loader2, ArrowLeft, ShieldCheck, Zap } from "lucide-react";
...
export default function Upgrade() {
  const [loading, setLoading] = useState(null); // stores plan name being upgraded to
  const { user, planName, refreshProfile } = useAuth();
  ...
  const handleUpgrade = async (plan) => {
    ...
    try {
      ...
      await axios.post(
        "https://ai-powered-mentor-platform.onrender.com/mock-upgrade",
        { plan },
        {
            ...
            >
              ...
              <div className="flex-1 space-y-4 mb-10">
                <div className={`p-4 rounded-2xl text-xs font-bold flex items-center gap-3 ${
                  tier.id === 'gold' ? 'bg-amber-500/10 text-amber-500' : (tier.id === 'silver' ? 'bg-indigo-500/10 text-indigo-500' : 'bg-zinc-800 text-zinc-400')
                }`}>
                  <ShieldCheck size={16} />
                  {tier.limit}
                </div>
                {tier.features.map((f, i) => (
                  <div key={i} className="flex items-center gap-3 text-zinc-400 text-sm pl-2">
              ...
              <button 
                ...
                disabled={!!loading || tier.isCurrent || tier.id === 'free'}
